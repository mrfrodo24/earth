...
var products = function() {
    ...
    var FACTORIES = {
        ...
        "temp": {
            ...
            create: function(attr) {
                return buildProduct({
                    ...
                    scale: scales.temperature(attr.surface, attr.level)
                });
            }
        },
        ...
        "geopotential": {
            matches: _.matches({param: "wind", overlayType: "geopotential"}),
            create: function(attr) {
                return buildProduct({
                    field: "scalar",
                    type: "geopotential",
                    description: localize({
                        name: {en: "Geopotential", ja: "ジオポテンシャル"},
                        qualifier: {en: " @ " + describeSurface(attr), ja: " @ " + describeSurfaceJa(attr)}
                    }),
                    paths: [gfs1p0degPath(attr, "geopotential", attr.surface, attr.level)],
                    date: gfsDate(attr),
                    builder: function(file) {
                        var record = file[0], data = record.data;
                        return {
